...
## Purpose
The person-to-subject relationship model — ownership, occupancy, and professional relationships
to a specific building-cluster subject (`Property`/`Building`/`Unit`/`Asset`), including the
ownership-verification lifecycle and access-control rules. `subject_type`/`subject_id` always
identify the target; `user_id` always identifies the person — the two must never be blurred, per
the real security bug this rule was recovered from (any-verified-owner-can-access-any-building).
