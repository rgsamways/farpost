...
describe("job_cost_breakdown", () => {
  afterEach(cleanup);

  it("rejects an invalid breakdown_type", async () => {
    const job = await insertTestJob();
    await expect(
      db.insert(schema.jobCostBreakdown).values({
        jobId: job.id,
        breakdownType: "bogus" as any,
      }),
    ).rejects.toThrow();
  });
  ...
  it("is deleted when its Job is deleted", async () => {
    const job = await insertTestJob();
    await db.insert(schema.jobCostBreakdown).values({ jobId: job.id, breakdownType: "estimate" });

    await db.delete(schema.job).where(eq(schema.job.id, job.id));

    const rows = await db
      .select()
      .from(schema.jobCostBreakdown)
      .where(eq(schema.jobCostBreakdown.jobId, job.id));
