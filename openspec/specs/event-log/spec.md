...
## Purpose
An append-only record of what happened, who did it, and what it's about (`Event`), with
per-recipient relevance and read-state tracked independently (`EventRecipient`) rather than as an
embedded list on the event itself. Foundation for `core-building-model.md`'s reputation-timeline
decision and any future notification fan-out.
