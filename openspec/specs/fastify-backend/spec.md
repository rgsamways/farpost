...
The running Fastify server, its Postgres connection, and its migration tooling — the foundation every later backend capability builds on. Established by `scaffold-fastify-backend` (archived 2026-07-26). No auth, no domain schema yet — see `wire-better-auth`.
