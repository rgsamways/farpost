...
`model` (text, nullable), `serial_number` (text, nullable), `location` (text, nullable — e.g.
...
#### Scenario: Asset carries location and last serviced date
- **WHEN** an `asset` row is inserted with `location` and `last_serviced_date` populated
- **THEN** both values persist and are independently readable, and `last_serviced_date` is
  distinct from `installed_date` on the same row
