...
## Purpose
Per-building, per-category tracking of when a fact was last documented and when it should be
considered stale again. Normalizes the old system's `dict[str, FactStaleness]` embedded on
`Building` into a real table, with `next_stale_at` computed by the database rather than trusted
from application code — making the old system's own "never trusted stale" guarantee airtight.
