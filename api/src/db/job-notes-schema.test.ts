...
describe("job_notes", () => {
  ...
  it("is deleted when its Job is deleted", async () => {
    ...
    await db.insert(schema.jobNotes).values({ jobId: job.id, requesterNotes: "Please hurry" });

    await db.delete(schema.job).where(eq(schema.job.id, job.id));

    const rows = await db.select().from(schema.jobNotes).where(eq(schema.jobNotes.jobId, job.id));
